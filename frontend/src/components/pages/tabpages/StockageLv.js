...
export default function StockageLv() {
    ...
    // lignes du tableau
    var rows = stockageaeralv

    var data_stockages = stockages

    const [action, setAction] = useState("create")
    const addStockageaeralv = ()=>{
        setForm(2)
        setAction("create")
    }
    const close = ()=>{
    ...
    const save = async ()=>{
        ...
        if(response.status == 201){
            ...
            data.stockage = data.stockageaera.name
            data.status = "EN ATTENTE APPROBATION"
            var d = []
            d.push(data)
            stockageaeralv.forEach(item=>{
                if(item.uuid != data.uuid){
                    d.push(item)
                }
            })
            setSeverity("success")
            setMessage("lv stockage aera has been added successfully")
            setOpen(true)
            setStockageaeralv(d)
        }else{
    ...
    const [message, setMessage] = useState("new lv for stockage aera is being added")
    const [severity, setSeverity] = useState("success")

    const disabled = (errorStockage | errorQuantity | errorVolume) ? true : false
    const [actionApprove, setActionApprove] = useState(false)

    const approveLvForm = (selection)=>{
        setForm(3)
        setUstockageid(selection.uuid)
        setUstockagename(selection.stockage)
    ...
    const saveApprove = async ()=>{
        ...
        if(response.status == 200){
            ...
            setSeverity("success")
            setMessage("lv stockage aera has been approved")
            setOpen(true)
            setStockageaeralv(d)
            setActionApprove(false)
        }
    ...
    const [ustockageid, setUstockageid] = useState()
    const [ustockagename, setUstockagename] = useState()
    ...
    if(isForm == 3){
        return(
            ...
            >
                <div>
                    <ControlTextInput callback={voidc}  disabled={true} id="ustockagename" label="Aire de Stockage" readOnly={true} required={true} type="text" value={ustockagename} />
                    ...
                    <ControlTextInput callback={voidc}  disabled={true} id="udemandvolume" label="Volume demandé" readOnly={true} required={true} type="text" value={udemandvolume} />
                    {
                        (actionApprove)?
                            <>
                                <TextInput required={true} id='quantity' error={errorAccordQuantity} label="Quantité Accordée" callback={change_accordquantity} value={quantity} />
                                <TextInput required={true} id='volume' error={errorAccordVolume} label="Volume Accordé" callback={change_accordvolume} value={volume} />
        ...
        <>
            ...
            <FloatingActionButton callback={addStockageaeralv} />
        </>
        );
    }
